...
ssize_t myfd_read(struct file *fp, char __user, size_t size, loff_t *offs)
{
    size_t t, i;
    char *tmp2;
...
ssize_t myfd_write(struct file *fp, const char __user, size_t size, loff_t *offs)
{
    ssize_t res = 0;

    rest = simple_write_to_buffer(str, size, offs, user, size) + 1;
    /*
     * 0x0 = '\0' 
     */
    str[size + 1] = 0x0;
